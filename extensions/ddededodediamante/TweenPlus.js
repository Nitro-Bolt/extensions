...
(function (Scratch) {
  ...
  class TweeningPlus {
    getInfo() {
      return {
        ...
        blocks: [
          ...
          {
            opcode: "tweenBranchStop",
            blockType: BlockType.COMMAND,
            text: Scratch.translate("end current tween"),
            isTerminal: true,
          },
          {
            opcode: "tweenBranchTimeElapsed",
            blockType: BlockType.REPORTER,
            text: Scratch.translate("current tween timer"),
            disableMonitor: true,
          },
        ],
        menus: {
    ...
    // Extra blocks from Tween+ below

    tweenBranchValue(_args, util) {
      const state = util.thread[ddeSymbol];
      if (!state) return 0;

      const progress = Math.min(state.timer.timeElapsed() / state.duration, 1);
      const tweened = state.easingFunction(progress, state.easeDirection);
      return interpolate(tweened, state.start, state.end);
    }

    tweenBranch(args, util) {
      if (!util.stackFrame.initialized) {
        ...
        let easingFunction = EasingMethods[easeMethod] || EasingMethods.linear;

        util.stackFrame.duration = Math.max(0, 1000 * Cast.toNumber(args.SEC));
        util.startStackTimer(util.stackFrame.duration);

        util.thread[ddeSymbol] = {
          easingFunction,
          easeDirection,
          start: Cast.toNumber(args.START),
          end: Cast.toNumber(args.END),
          timer: util.stackFrame.timer,
          duration: util.stackFrame.duration,
          endCurrent: false,
        };
      }

      const endCurrent = util.thread[ddeSymbol].endCurrent;
    ...
    tweenBranchStop(_args, util) {
      if (!util.thread[ddeSymbol]) return;
      util.thread[ddeSymbol].endCurrent = true;
    }
