...
(function (Scratch) {
  ...
  const getPenFramebufferInfo = function (framebuffer) {
    const penSkinIds = renderer._penSkinIds || [renderer._penSkinId];
    for (const skinId of penSkinIds) {
  ...
  const patchPenSkin = function (skin) {
    if (skin._clipBlendPatched) return;
    skin._clipBlendPatched = true;
    const exit = skin._lineOnBufferDrawRegionId.exit;
    skin._lineOnBufferDrawRegionId.exit = () => {
      exit();
      resetPenModes();
    };
  };
  const patchPenExtension = function () {
    const ext_pen = runtime.ext_pen;
    if (!ext_pen || ext_pen._clipBlendPatched) return;
    ext_pen._clipBlendPatched = true;
  ...
  const PenSkin = renderer.exports.PenSkin;
  const flushLines = PenSkin.prototype._flushLines;
  PenSkin.prototype._flushLines = function () {
    setupModes(lastClipbox, lastBlendMode, true);
    flushLines.call(this);
  };

  for (const skin of renderer._allSkins) {
    if (skin instanceof PenSkin) patchPenSkin(skin);
  }
  patchPenExtension();

  // Pen papers create skins lazily, so keep patching future Pen skins too.
  const createPenSkin = renderer.createPenSkin;
  renderer.createPenSkin = function (...args) {
    const skinId = createPenSkin.apply(this, args);
    patchPenSkin(renderer._allSkins[skinId]);
    patchPenExtension();
    return skinId;
  };
